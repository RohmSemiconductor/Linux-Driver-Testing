...
/*
 * Rude and crude test code for adding a DT overlay blob in running
 * kernel. Bugs here and there - does just minimum of what is required
 * for my test environment - and that's not much.
 *
 * You probably don't want to use this code. If you search for a better
 * (and more professional, better maintained and documented and more widely
 * used) overlay adding implementation - then I suggest you to go look for
 * overlay mechanisms maintained by Geert in Renesas tree.
 *
...
static int create_overlay(void *blob, ssize_t size)
{
	...
	#else
	if((err=of_overlay_fdt_apply(blob, size,  &ov_id, NULL)))
	#endif
#endif
