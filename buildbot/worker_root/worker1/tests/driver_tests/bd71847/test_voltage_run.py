...
def test_bd71847(command):
    for regulator in bd71847.board.data['regulators'].keys():
        ...
        if "volt_change_not_allowed_while_on" in bd71847.board.data['regulators'][regulator]:
            print("Alright this works")
        
        else:
            bd71847.regulator_voltage_run(regulator,command)
            #regulator_voltage = bd71847.regulator_voltage(regulator,command)





            #            assert regulator_voltage == bd71847.board.data['regulators'][regulator]['regulator_en_bitmask']
            # 
